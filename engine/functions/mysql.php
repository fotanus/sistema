...
class MySQL {
    ...
    /* private $db_host    = "localhost";  // server name
      private $db_user    = "motivarebasic";       // user name
      private $db_pass    = "t5#r4qw2gFG";           // password
      private $db_dbname  = "colegiomotivare_motivare";           // database name */
    private $db_host = "192.168.1.122";  // server name
    private $db_user = "kapi";       // user name
    private $db_pass = "ylmd25oi";           // password
    private $db_dbname = "sistema";           // database name
     ...
     */
    public function GetHTML($showCount = true, $msgNoRecords = "No existen registros.") {
        global $config;
        if ($this->last_result) {
            if ($this->RowCount() > 0) {
                $html = "";
                ...
                while ($member = mysql_fetch_object($this->last_result)) {
                    ...
                    $html .= "\t<tr>\n";
                    foreach ($member as $key => $value) {
                        if ($key == "Número Usuario" || $key == "Id") {
                            $html .= "<input type=\"hidden\" value=\"$value\"/>";
                        }
                        if ($key == "Nombre" || $key == "Titulo") {
                            $html .= "\t\t<td class=\"nameClick\"><a href=\"#\" class=\"clicName\">" . htmlspecialchars($value) . "</a></td>\n";
                        } else if ($key == "Descripcion"){
                            $html .= "\t\t<td class=\"$key\">" . $value . "</td>\n";
                        } else if ($key == "Imagen") {
                            $html .= "\t\t<td class=\"$key\">";
                            $html .= "<input type=\"hidden\" value=\"$value\"/>";
                            $html .= "<img src=\"" . $config['functions']['url']->returnPath('img/nye/thumb/') . $value . "\" alt=\"\"></td>\n";
                        } else {
                            $html .= "\t\t<td class=\"$key\">" . htmlspecialchars($value) . "</td>\n";
                        }
                    }
                    $html .= "\t</tr>\n";
                }
     ...
     */
    public function SeekPosition($val = NULL) {
        if ($val != NULL) {
            $this->active_row = $val;
        }
